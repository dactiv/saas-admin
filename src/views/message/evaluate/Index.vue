<template>
  <div>
    <a-breadcrumb class="hidden-xs hidden-sm hidden-md">
      ...
      <a-breadcrumb-item><icon-font class="icon" type="icon-excellent" /> 评价消息</a-breadcrumb-item>
    </a-breadcrumb>

    <a-card title="评价消息" class="basic-box-shadow margin-lg-top">

      <template #extra>
        <icon-font class="icon" type="icon-excellent" />
      </template>

      <a-input v-model:value="this.form['filter_[target_name_like]']" @pressEnter="this.search(1)"  placeholder="请输入目标名称进行查询" class="margin-lg-bottom">
        <template #addonAfter>
      ...
      <a-table :loading="this.loading" class="ant-table-striped" :row-selection="{ selectedRowKeys: selectedIds, onChange:this.selectChange }" :rowKey="record=>record.id" :scroll="{ x: this.columns.reduce((sum, c) => sum + c.width, 0) }" :pagination="false" :data-source="this.page.elements" :columns="this.columns" bordered>
        <template #bodyCell="{column, record}">
          ...
          <template v-if="column.dataIndex === 'rate'">
            <a-rate v-model:value="record.rate" disabled allow-half />
          </template>
...
export default {
  ...
  created() {
    this.loadConfigEnumerate({service:"message", enumerateName:"EvaluateMessageTypeEnum"}).then((r) => this.targetTypeOptions = r.data.data);
  ...
  methods:{
    ...
    remove(ids) {
      ...
      this
          .$http
          .post("/api/message/evaluate/delete",this.formUrlEncoded({ids:ids})).then((r) => {
    ...
    search(number) {
      ...
      this
          .$http
          .post("/api/message/evaluate/page",this.formUrlEncoded(param))
          .then(r => {
