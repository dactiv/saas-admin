<template>
  <div>
    <a-breadcrumb class="hidden-xs hidden-sm hidden-md">
      ...
      <a-breadcrumb-item><icon-font class="icon" type="icon-enum-major-o" /> 我经办的</a-breadcrumb-item>
    </a-breadcrumb>

    <a-card title="我经办的" class="basic-box-shadow margin-lg-top">
...
export default {
  ...
  methods:{
    detail(record) {
      ...
      if (record !== undefined) {
        to["query"] = {id:JSON.stringify([record.apply.id])};
      }
    ...
    search(number) {
      ...
      this
          .$http
          .post("/api/workflow/work/myProcessed",this.formUrlEncoded(param))
          .then(r => {
