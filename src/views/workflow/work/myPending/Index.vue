<template>
  <div>
    <a-breadcrumb class="hidden-xs hidden-sm hidden-md">
      ...
      <a-breadcrumb-item><icon-font class="icon" type="icon-more" /> 我的待办</a-breadcrumb-item>
    </a-breadcrumb>

    <a-card title="我的待办" class="basic-box-shadow margin-lg-top">
      ...
      <a-input v-model:value="this.form['filter_[name]']" @pressEnter="this.search(1)" placeholder="请输入表单名称进行查询" class="margin-lg-bottom">
        ...
        <template #addonBefore>
          <a-button type="text" size="small" @click="approval(this.selectedIds)">
            <icon-font class="icon" type="icon-ashbin" />
            <span class="hidden-xs hidden-sm hidden-md">处理选中</span>
          </a-button>
        </template>
      </a-input>

      <a-table :loading="this.loading" class="ant-table-striped" :row-selection="{ selectedRowKeys: selectedIds, onChange:this.selectChange }" :rowKey="record=>record.apply.id" :scroll="{ x: this.columns.reduce((sum, c) => sum + c.width, 0) }" :pagination="false" :data-source="this.page.elements" :columns="this.columns" bordered>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <div class="text-center">
              <a-button size="small" type="primary" @click="approval([record.apply.id], record.name)">
                <icon-font class="icon" type="icon-ashbin" />
                <span class="hidden-xs hidden-sm hidden-md">处理</span>
              </a-button>
            </div>
          </template>
        </template>
...
export default {
  ...
  methods:{
    ...
    approval(ids) {
      //sessionStorage.setItem(process.env.VUE_APP_SESSION_STORAGE_WORK_APPROVAL, JSON.stringify(ids));
      this.$router.push({name: "workflow_apply_detail", query:{id:JSON.stringify(ids)}});
    },
    search(number) {
      ...
      this
          .$http
          .post("/api/workflow/work/myPending",this.formUrlEncoded(param))
          .then(r => {
